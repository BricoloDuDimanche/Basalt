...
class DeezerSourceManager : AbstractSourceLoader() {
    ...
    @Suppress("PrivatePropertyName")
    private val ALBUM_PATTERN = Pattern.compile("^https?://.*\\.deezer\\.com/(?:[a-z]{2}/)?album/([0-9]+).*")

    @Suppress("PrivatePropertyName")
    private val TRACK_PATTERN = Pattern.compile("^https?://.*\\.deezer\\.com/(?:[a-z]{2}/)?track/([0-9]+).*")

    override fun getSourceName() = "deezer"
    ...
    private fun loadTrack(manager: DefaultAudioPlayerManager, match: Matcher): AudioItem {
        ...
        val track = Basalt.deezer.getTrack(trackId) ?: return AudioReference.NO_TRACK
